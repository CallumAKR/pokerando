...
/* Each of these enums corresponds to an index in the list of TMs + HMs item ids in
 * gTMHMItemMoveIds. The index for an item can be retrieved with GetItemTMHMIndex below.
 * TM and HM names deliberately use separate enum prefixes because later-generation
 * catalogues can contain a TM for a move that remains an Emerald HM.
 */
#define UNPACK_TM_ENUM(_tm) CAT(ENUM_TM_, _tm),
#define UNPACK_HM_ENUM(_hm) CAT(ENUM_HM_, _hm),
enum TMHMIndex
{
...
#define UNPACK_ITEM_TO_TM_INDEX(_tm) case CAT(ITEM_TM_, _tm): return CAT(ENUM_TM_, _tm) + 1;
#define UNPACK_ITEM_TO_HM_INDEX(_hm) case CAT(ITEM_HM_, _hm): return CAT(ENUM_HM_, _hm) + 1;
...
{
    // A move can appear in both lists (for example, USUM TM94 Surf and
    // Emerald HM03 Surf). TMs are stored first, so return the active TM when
    // one exists and otherwise return the HM.
    for (enum TMHMIndex index = 1; index <= NUM_ALL_MACHINES; index++)
    {
