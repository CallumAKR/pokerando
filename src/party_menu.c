...
struct PartyMenuInternal
{
    ...
    u32 messageId:14;
    u8 windowId[3];
    u8 actions[9];  // Summary, four field moves, Level to cap, Switch, Item, Cancel
    u8 numActions;
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    case SELECTWINDOW_ACTIONS:
        SetWindowTemplateFields(&window, 2, 19, 19 - (sPartyMenuInternal->numActions * 2), 12, sPartyMenuInternal->numActions * 2, 14, 0x2E9);
        break;
...
{
    // Use the existing Cap Candy level-up, move-learning and evolution flow.
    // This party action does not consume an item or open the Bag.
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[0]);
    PartyMenuRemoveWindow(&sPartyMenuInternal->windowId[1]);
...
static void Task_LearnNextMoveOrClosePartyMenu(u8 taskId)
{
    ...
    {
        // data1 contains the move ID. learnMoveState records whether this
        // prompt belongs to a multi-level level-up sequence such as Cap Candy.
...
void ItemUseCB_CapCandy(u8 taskId, TaskFunc task)
{
    ...
    // Empty slots, Eggs, and permanently dead Pokémon cannot use Cap Candy.
    if (species == SPECIES_NONE
    ...
    // Save the old level/stats for the normal level-up stat pages.
    sInitialLevel = currentLevel;
    BufferMonStatsToTaskData(mon, arrayPtr);

    // Move the Pokémon directly to the exact EXP threshold for the current cap.
    ...
    // Refresh the party display exactly like Rare Candy does.
    PlaySE(SE_SELECT);
    gPartyMenuUseExitCallback = TRUE;
    UpdateMonDisplayInfoAfterRareCandy(gPartyMenu.slotId, mon);
    GetMonNickname(mon, gStringVar1);
    ...
    // Reuse the existing Rare Candy pipeline:
    // stat pages -> every skipped level-up move -> evolution check.
    gTasks[taskId].func = Task_DisplayLevelUpStatsPg1;
}
