...
{
    // RANDOMIZER GAME OPTIONS: POKEMON DATA OVERRIDES
...
void SetMonData(struct Pokemon *mon, s32 field, const void *dataArg)
{
    ...
    // RANDOMIZER GAME OPTIONS: PERMADEATH HP HANDLING
    #if RANDOMIZER_PERMADEATH
    // A non-Egg Pokémon that reaches 0 HP is permanently dead.
    if (mon->hp == 0
     && GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE
     && GetMonData(mon, MON_DATA_SPECIES) != SPECIES_EGG)
    {
        isDead = TRUE;
        SetBoxMonData(&mon->box, MON_DATA_IS_DEAD, &isDead);
    }

    // Dead Pokémon can never have their HP restored.
    if (GetBoxMonData(&mon->box, MON_DATA_IS_DEAD))
        mon->hp = 0;
    #endif
...
#if RANDOMIZER_REGIONAL_EVOLUTION_POSTCARDS
    enum Region evolutionRegion = GetCurrentRegion();

    switch (heldItem)
    {
    case ITEM_ALOLA_POSTCARD:
        evolutionRegion = REGION_ALOLA;
        break;
    case ITEM_GALAR_POSTCARD:
        evolutionRegion = REGION_GALAR;
        break;
    case ITEM_HISUI_POSTCARD:
        evolutionRegion = REGION_HISUI;
        break;
    default:
        break;
    }
...
static enum Species GetPermanentMegaEvolutionTarget(enum Species species, enum Item evolutionItem)
{
    ...
    for (u32 i = 0; formChanges != NULL && formChanges[i].method != FORM_CHANGE_TERMINATOR; i++)
    {
        if (formChanges[i].method == FORM_CHANGE_BATTLE_MEGA_EVOLUTION_ITEM
            ...
            && gSpeciesInfo[SanitizeSpeciesId(formChanges[i].targetSpecies)].isMegaEvolution)
            return formChanges[i].targetSpecies;
    }
#endif
...
enum Species GetEvolutionTargetSpecies(struct Pokemon *mon, enum EvolutionMode mode, u16 evolutionItem, struct Pokemon *tradePartner, bool32 *canStopEvo, enum EvoState evoState)
{
    ...
    {
        targetSpecies = GetPermanentMegaEvolutionTarget(species, evolutionItem);
        if (targetSpecies != SPECIES_NONE)
        {
            if (canStopEvo != NULL)
                *canStopEvo = FALSE;
...
#if RANDOMIZER_PERMANENT_MEGAS
    // Permanent Megas start battle as Mega species and therefore have no
    // saved pre-Mega species to restore after fainting or battle end.
    if ((method == FORM_CHANGE_END_BATTLE || method == FORM_CHANGE_FAINT)
        && gSpeciesInfo[SanitizeSpeciesId(currentSpecies)].isMegaEvolution
