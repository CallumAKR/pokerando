...
bool32 CanMegaEvolve(enum BattlerId battler)
{
    ...
    // Permanent Mega Stones are party-menu evolution items, not battle gimmicks.
...
#if RANDOMIZER_PERMANENT_MEGAS
    // A Pokémon that entered battle as a Mega has no pre-Mega species saved.
    // Keep that permanent species on faint/end-battle form-change passes.
    struct PartyState *permanentMegaState = GetBattlerPartyState(battler);
    if ((method == FORM_CHANGE_END_BATTLE || method == FORM_CHANGE_FAINT)
        && IsBattlerMegaEvolved(battler)
        && (permanentMegaState == NULL || permanentMegaState->changedSpecies == SPECIES_NONE))
        return FALSE;
#endif
...
{
    // RANDOMIZER DIFFICULTY OPTIONS: DISABLE BAG IN TRAINER BATTLES
