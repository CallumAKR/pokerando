...
static const u8 sText_PartyRestored[] = _("The living party was\nfully restored!{PAUSE_UNTIL_PRESS}");
static const u8 sText_PartyAlreadyHealthy[] = _("The living party is\nalready healthy.{PAUSE_UNTIL_PRESS}");
static const u8 sText_PermaRepelOn[] = _("Permanent Repel was\nturned ON.{PAUSE_UNTIL_PRESS}");
static const u8 sText_PermaRepelOff[] = _("Permanent Repel was\nturned OFF.{PAUSE_UNTIL_PRESS}");
static const u8 sText_TimeSetToDay[] = _("The time was changed\nto daytime.{PAUSE_UNTIL_PRESS}");
static const u8 sText_TimeSetToEvening[] = _("The time was changed\nto evening.{PAUSE_UNTIL_PRESS}");
static const u8 sText_TimeSetToNight[] = _("The time was changed\nto nighttime.{PAUSE_UNTIL_PRESS}");
static const u8 sText_WeatherChanged[] = _("The local weather was\nchanged.{PAUSE_UNTIL_PRESS}");
static const u8 sText_FieldMoveLocked[] = _("You haven't unlocked\nthat field ability yet.{PAUSE_UNTIL_PRESS}");
static const u8 sText_FieldMoveCantUse[] = _("That can't be used\nhere right now.{PAUSE_UNTIL_PRESS}");
...
void ItemUseOutOfBattle_PartyRestorer(u8 taskId)
{
    // RANDOMIZER GAME OPTIONS: CUSTOM FIELD ITEMS: ItemUseOutOfBattle_PartyRestorer
    ...
    if (!gTasks[taskId].tUsingRegisteredKeyItem)
    {
        DisplayItemMessage(
            taskId,
            FONT_NORMAL,
            healedAny ? sText_PartyRestored : sText_PartyAlreadyHealthy,
            CloseItemMessage
        );
    }
    else
...
{
    // RANDOMIZER GAME OPTIONS: CUSTOM FIELD ITEMS: ItemUseOutOfBattle_CapCandy
...
{
    // RANDOMIZER GAME OPTIONS: CUSTOM FIELD ITEMS: ItemUseOutOfBattle_PermaRepel
    ...
    FlagToggle(FLAG_SYS_PERMA_REPEL);

    enabled = FlagGet(FLAG_SYS_PERMA_REPEL);

    PlaySE(enabled ? SE_EXP_MAX : SE_PC_OFF);
    ...
    if (!gTasks[taskId].tUsingRegisteredKeyItem)
    {
        DisplayItemMessage(
            taskId,
            FONT_NORMAL,
            enabled ? sText_PermaRepelOn : sText_PermaRepelOff,
            CloseItemMessage
        );
    }
    else
...
enum UtilityItemMenuKind
{
    UTILITY_MENU_TIME,
    UTILITY_MENU_WEATHER,
};

enum UtilityItemMenuChoice
{
    UTILITY_TIME_DAY = 12,
    UTILITY_TIME_EVENING = 19,
    UTILITY_TIME_NIGHT = 22,
    UTILITY_MENU_CANCEL = 0x7FFF,
};

static const struct ListMenuItem sTimeTurnerMenuItems[] =
{
    {COMPOUND_STRING("Day"), UTILITY_TIME_DAY},
    {COMPOUND_STRING("Evening"), UTILITY_TIME_EVENING},
    {COMPOUND_STRING("Night"), UTILITY_TIME_NIGHT},
    {gText_Cancel2, UTILITY_MENU_CANCEL},
};

// Only concrete, generally safe overworld weather is exposed here. Internal
// controller states (underwater, abnormal, route-cycle and dynamic weather)
// are deliberately omitted because they are map/story-specific.
static const struct ListMenuItem sWeatherSetterMenuItems[] =
{
    {COMPOUND_STRING("Clear"), WEATHER_NONE},
    {COMPOUND_STRING("Clouds"), WEATHER_SUNNY_CLOUDS},
    {COMPOUND_STRING("Sunny"), WEATHER_SUNNY},
    {COMPOUND_STRING("Rain"), WEATHER_RAIN},
    {COMPOUND_STRING("Snow"), WEATHER_SNOW},
    {COMPOUND_STRING("Thunderstorm"), WEATHER_RAIN_THUNDERSTORM},
    {COMPOUND_STRING("Fog"), WEATHER_FOG_HORIZONTAL},
    {COMPOUND_STRING("Volcanic Ash"), WEATHER_VOLCANIC_ASH},
    {COMPOUND_STRING("Sandstorm"), WEATHER_SANDSTORM},
    {COMPOUND_STRING("Diagonal Fog"), WEATHER_FOG_DIAGONAL},
    {COMPOUND_STRING("Shade"), WEATHER_SHADE},
    {COMPOUND_STRING("Drought"), WEATHER_DROUGHT},
    {COMPOUND_STRING("Downpour"), WEATHER_DOWNPOUR},
    {gText_Cancel2, UTILITY_MENU_CANCEL},
};

static const struct WindowTemplate sTimeTurnerWindowTemplate =
{
    .bg = 0,
    .tilemapLeft = 17,
    .tilemapTop = 1,
...
static const struct WindowTemplate sWeatherSetterWindowTemplate =
{
    .bg = 0,
    .tilemapLeft = 11,
    .tilemapTop = 1,
...
static const struct ListMenuTemplate sUtilityItemListMenuTemplate =
{
...
#define tUtilityListTaskId data[0]
#define tUtilityWindowId   data[1]
#define tUtilityMenuKind   data[2]

static void CloseUtilityItemMenu(u8 taskId, bool32 showMessage, const u8 *message)
{
    DestroyListMenuTask(gTasks[taskId].tUtilityListTaskId, NULL, NULL);
    ClearStdWindowAndFrame(gTasks[taskId].tUtilityWindowId, TRUE);
    RemoveWindow(gTasks[taskId].tUtilityWindowId);
...
static void Task_HandleUtilityItemMenuInput(u8 taskId)
{
    s32 input = ListMenu_ProcessInput(gTasks[taskId].tUtilityListTaskId);

    if (input == LIST_NOTHING_CHOSEN)
        return;

    if (input == LIST_CANCEL || input == UTILITY_MENU_CANCEL)
    {
        PlaySE(SE_SELECT);
        CloseUtilityItemMenu(taskId, FALSE, NULL);
        return;
    }

    PlaySE(SE_SELECT);

    if (gTasks[taskId].tUtilityMenuKind == UTILITY_MENU_TIME)
    {
        SetTimeOfDay(input);
        UpdateTimeOfDay(TRUE);
        if (input == UTILITY_TIME_DAY)
            CloseUtilityItemMenu(taskId, TRUE, sText_TimeSetToDay);
        else if (input == UTILITY_TIME_EVENING)
            CloseUtilityItemMenu(taskId, TRUE, sText_TimeSetToEvening);
        else
            CloseUtilityItemMenu(taskId, TRUE, sText_TimeSetToNight);
    }
    else
    {
        SetWeather(input);
        CloseUtilityItemMenu(taskId, TRUE, sText_WeatherChanged);
    }
}

static void ShowUtilityItemMenu(u8 taskId, enum UtilityItemMenuKind kind)
{
    struct ListMenuTemplate menuTemplate = sUtilityItemListMenuTemplate;
    ...
    gTasks[taskId].tUtilityMenuKind = kind;

    if (kind == UTILITY_MENU_TIME)
    {
        windowTemplate = &sTimeTurnerWindowTemplate;
        menuTemplate.items = sTimeTurnerMenuItems;
        menuTemplate.totalItems = ARRAY_COUNT(sTimeTurnerMenuItems);
        menuTemplate.maxShowed = ARRAY_COUNT(sTimeTurnerMenuItems);
    }
    else
    {
        windowTemplate = &sWeatherSetterWindowTemplate;
        menuTemplate.items = sWeatherSetterMenuItems;
        menuTemplate.totalItems = ARRAY_COUNT(sWeatherSetterMenuItems);
        menuTemplate.maxShowed = 6;
    }

    gTasks[taskId].tUtilityWindowId = AddWindow(windowTemplate);
    DrawStdWindowFrame(gTasks[taskId].tUtilityWindowId, FALSE);
    menuTemplate.windowId = gTasks[taskId].tUtilityWindowId;
    gTasks[taskId].tUtilityListTaskId = ListMenuInit(&menuTemplate, 0, 0);
    CopyWindowToVram(gTasks[taskId].tUtilityWindowId, COPYWIN_FULL);
    gTasks[taskId].func = Task_HandleUtilityItemMenuInput;
}

static void ItemUseOnFieldCB_TimeTurner(u8 taskId)
{
    ShowUtilityItemMenu(taskId, UTILITY_MENU_TIME);
}

static void ItemUseOnFieldCB_WeatherSetter(u8 taskId)
{
    ShowUtilityItemMenu(taskId, UTILITY_MENU_WEATHER);
}

void ItemUseOutOfBattle_TimeTurner(u8 taskId)
{
    // RANDOMIZER GAME OPTIONS: CUSTOM FIELD ITEMS: ItemUseOutOfBattle_TimeTurner
#if !RANDOMIZER_TIME_TURNER
    DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    return;
#endif
    sItemUseOnFieldCB = ItemUseOnFieldCB_TimeTurner;
    SetUpItemUseOnFieldCallback(taskId);
}

void ItemUseOutOfBattle_WeatherSetter(u8 taskId)
{
    // RANDOMIZER GAME OPTIONS: CUSTOM FIELD ITEMS: ItemUseOutOfBattle_WeatherSetter
#if !RANDOMIZER_WEATHER_SETTER
    DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
    return;
#endif
    sItemUseOnFieldCB = ItemUseOnFieldCB_WeatherSetter;
    SetUpItemUseOnFieldCallback(taskId);
}

#undef tUtilityListTaskId
#undef tUtilityWindowId
#undef tUtilityMenuKind



static bool32 TryUseFieldTool(u8 taskId, u8 fieldMove)
{
    if (!IsFieldMoveUnlocked(fieldMove))
    {
        if (!gTasks[taskId].tUsingRegisteredKeyItem)
            DisplayItemMessage(taskId, FONT_NORMAL, sText_FieldMoveLocked, CloseItemMessage);
        else
            DisplayItemMessageOnField(taskId, sText_FieldMoveLocked, Task_CloseCantUseKeyItemMessage);
    ...
    {
        if (!gTasks[taskId].tUsingRegisteredKeyItem)
            DisplayItemMessage(taskId, FONT_NORMAL, sText_FieldMoveCantUse, CloseItemMessage);
        else
            DisplayItemMessageOnField(taskId, sText_FieldMoveCantUse, Task_CloseCantUseKeyItemMessage);
...
{
    // RANDOMIZER GAME OPTIONS: HM-FREE FIELD TOOLS: ItemUseOutOfBattle_FlashTool
    ...
    if (TryUseFieldTool(taskId, FIELD_MOVE_FLASH))
        SetUpItemUseCallback(taskId);
}

void ItemUseOutOfBattle_DigTool(u8 taskId)
{
    // RANDOMIZER GAME OPTIONS: HM-FREE FIELD TOOLS: ItemUseOutOfBattle_DigTool
#if !RANDOMIZER_HM_FREE_FIELD_MOVES
    DisplayDadsAdviceCannotUseItemMessage(
        taskId,
        gTasks[taskId].tUsingRegisteredKeyItem
    ...
    if (TryUseFieldTool(taskId, FIELD_MOVE_DIG))
        SetUpItemUseCallback(taskId);
}

void ItemUseOutOfBattle_TeleportTool(u8 taskId)
{
    // RANDOMIZER GAME OPTIONS: HM-FREE FIELD TOOLS: ItemUseOutOfBattle_TeleportTool
#if !RANDOMIZER_HM_FREE_FIELD_MOVES
    DisplayDadsAdviceCannotUseItemMessage(
        taskId,
        gTasks[taskId].tUsingRegisteredKeyItem
    ...
    if (TryUseFieldTool(taskId, FIELD_MOVE_TELEPORT))
        SetUpItemUseCallback(taskId);
}

void ItemUseOutOfBattle_FlyTool(u8 taskId)
{
    // RANDOMIZER GAME OPTIONS: HM-FREE FIELD TOOLS: ItemUseOutOfBattle_FlyTool
#if !RANDOMIZER_HM_FREE_FIELD_MOVES
    DisplayDadsAdviceCannotUseItemMessage(
        taskId,
        gTasks[taskId].tUsingRegisteredKeyItem
    ...
    if (!SetUpFieldMove(FIELD_MOVE_FLY))
    {
        if (!gTasks[taskId].tUsingRegisteredKeyItem)
            DisplayItemMessage(taskId, FONT_NORMAL, sText_FieldMoveCantUse, CloseItemMessage);
        else
