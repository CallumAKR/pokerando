...
bool8 StandardWildEncounter(u16 curMetatileBehavior, u16 prevMetatileBehavior)
{
    ...
    // RANDOMIZER GAME OPTIONS: PERMA REPEL ENCOUNTER CHECK
    if (RANDOMIZER_PERMA_REPEL && FlagGet(FLAG_SYS_PERMA_REPEL))
        return FALSE;
    ...
    else
    {
        if (MetatileBehavior_IsLandWildEncounter(curMetatileBehavior) == TRUE)
        {
            ...
            {
                if (FALSE && DoMassOutbreakEncounterTest() == TRUE && SetUpMassOutbreakEncounter(WILD_CHECK_REPEL | WILD_CHECK_KEEN_EYE) == TRUE)
                {
                    BattleSetup_StartWildBattle();
                    return TRUE;
                }
