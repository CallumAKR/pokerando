...
#define UNPACK_TM_ITEM_ID(_tm) [CAT(ENUM_TM_, _tm) + 1] = { CAT(ITEM_TM_, _tm), CAT(MOVE_, _tm) },
#define UNPACK_HM_ITEM_ID(_hm) [CAT(ENUM_HM_, _hm) + 1] = { CAT(ITEM_HM_, _hm), CAT(MOVE_, _hm) },
...
// Mega Stones are ordinary held items in the unmodified game.  When the
// randomizer's permanent-Mega option is enabled they must behave like
// evolution stones in the Bag, without changing their normal data when the
// option is disabled.
static bool32 IsPermanentMegaStone(enum Item itemId)
{
