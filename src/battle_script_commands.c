        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gParties[B_TRAINER_PLAYER][*expMonId]))
                {
                    ...
                    // A hard-capped Pokémon receives zero EXP. Do not send a
                    // zero-length update to the EXP bar: its animation waits
                    // for movement and would never finish. EVs are still
                    // awarded exactly as they are for other battle participants.
...
{
    // RANDOMIZER GAME OPTIONS: GUARANTEED CAPTURE
