...
// Trainer Hill's original prize tables refer directly to specific TM item
// constants.  Selectable TM catalogues legitimately omit some of those
// moves, so store the move in the table and resolve it to the active TM item
// at runtime instead.  If the move is not a TM in the selected catalogue,
// give a Rare Candy rather than an invalid item.
#define TRAINER_HILL_TM_PRIZE_FLAG       (1 << 15)
#define TRAINER_HILL_TM_PRIZE(move)      (TRAINER_HILL_TM_PRIZE_FLAG | (move))
#define TRAINER_HILL_TM_PRIZE_FALLBACK   ITEM_RARE_CANDY
...
static const u16 sPrizeListRoar[]        = {TRAINER_HILL_TM_PRIZE(MOVE_ROAR),        ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListSludgeBomb[]  = {TRAINER_HILL_TM_PRIZE(MOVE_SLUDGE_BOMB), ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListToxic[]       = {TRAINER_HILL_TM_PRIZE(MOVE_TOXIC),       ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListSunnyDay[]    = {TRAINER_HILL_TM_PRIZE(MOVE_SUNNY_DAY),   ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListEarthQuake[]  = {TRAINER_HILL_TM_PRIZE(MOVE_EARTHQUAKE),  ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
...
static const u16 sPrizeListBrickBreak[]  = {TRAINER_HILL_TM_PRIZE(MOVE_BRICK_BREAK), ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListTorment[]     = {TRAINER_HILL_TM_PRIZE(MOVE_TORMENT),     ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListSkillSwap[]   = {TRAINER_HILL_TM_PRIZE(MOVE_SKILL_SWAP),  ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListGigaDrain[]   = {TRAINER_HILL_TM_PRIZE(MOVE_GIGA_DRAIN),  ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
static const u16 sPrizeListAttract[]     = {TRAINER_HILL_TM_PRIZE(MOVE_ATTRACT),     ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
...
static u16 GetPrizeItemId(void)
{
    ...
    prize = prizeList[id];
    if (prize & TRAINER_HILL_TM_PRIZE_FLAG)
    {
        tmItem = GetTMHMItemIdFromMoveId((enum Move)(prize & ~TRAINER_HILL_TM_PRIZE_FLAG));
        if (tmItem != ITEM_NONE)
            return tmItem;

        return TRAINER_HILL_TM_PRIZE_FALLBACK;
    }
